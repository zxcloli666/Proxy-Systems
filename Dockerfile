...
#   `builder`        — alpine/musl, builds every binary except simple-proxy.
#   `builder-simple` — debian/glibc, builds simple-proxy only. Its wreq
#                      impersonation pulls BoringSSL, whose bindgen step needs
#                      to dlopen libclang — impossible in a static musl build.
...
RUN --mount=type=cache,target=/usr/local/cargo/registry,sharing=locked \
    ...
    && cargo build --release --bins --workspace --exclude simple-proxy || true
...
RUN --mount=type=cache,target=/usr/local/cargo/registry,sharing=locked \
    ...
    && cargo build --release --bins --workspace --exclude simple-proxy \
    && mkdir -p /out \
    && cp target/release/simple-ipv6-proxy \
...
RUN --mount=type=cache,target=/usr/local/cargo/registry,sharing=locked \
    ...
    && echo "fn main(){}" > tor-proxy/src/main.rs \
    && touch proxy-common/src/lib.rs \
    && cargo build --release -p simple-proxy || true

COPY . .
RUN --mount=type=cache,target=/usr/local/cargo/registry,sharing=locked \
    --mount=type=cache,target=/usr/local/cargo/git,sharing=locked \
    touch proxy-common/src/lib.rs simple-proxy/src/main.rs \
    && cargo build --release -p simple-proxy \
    && mkdir -p /out \
    && cp target/release/simple-proxy /out/
